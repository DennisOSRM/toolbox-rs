...
impl<T: Ord + Copy> StaticGraph<T> {
  ...
  pub const INVALID_EDGE_ID: EdgeID = EdgeID::MAX;
    ...
    pub fn cycle_check(&self) -> bool {
        let mut subtree_root_nodes = Vec::new();
        subtree_root_nodes.resize(self.number_of_nodes(), NodeID::MAX);
        let mut stack = Vec::new();

        for root in self.node_range() {
            if subtree_root_nodes[root as usize] != NodeID::MAX {
                continue;
            }

            stack.push(root);
            while let Some(node) = stack.pop() {
                // marking node as explored from its root
                subtree_root_nodes[node as usize] = root;
                for edge in self.edge_range(node) {
                    // push unvisited children to stack
                    let target = self.target(edge);
                    if subtree_root_nodes[target as usize] == subtree_root_nodes[root as usize] {
                        // cycle detected: target has the same root as its parent
                        return false;
                    }
                    if subtree_root_nodes[target as usize] == NodeID::MAX {
                        stack.push(target);
                    }
                }
            }
