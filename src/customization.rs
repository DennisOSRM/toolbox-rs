//! The customization of a partitioned graph: what it costs to cross each of
//! its cells, worked out level by level.
//!
//! A cell is summarized by the distances between the nodes on its border, as
//! that is all a path through it can do. The finest level is worked out on the
//! graph, and every level above it out of the cells below rather than out of
//! the graph again: a path through a coarse cell crosses a cell below from one
//! of its border nodes to another, which is already tabulated, and then takes
//! an arc into the next one. That is what keeps a cell of a million nodes
//! affordable.
//!
//! Cells are worked out when they are first asked about and kept afterwards,
//! so a caller that only wants one cell does not pay for the rest of its
//! level. [`Customization::forget`] drops what has been kept, for a caller
//! that walks a whole level and has no use for the tables below it.
...
/// The cells of a partition and what it costs to cross them.
...
mod tests {
    ...
    fn a_cell_without_a_border_is_not_tabulated() {
        ...
        // and there is nothing to check either, rather than nothing wrong
        let check = customization.check(0, 0);
        assert!(!check.has_border);
